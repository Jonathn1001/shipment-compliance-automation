...
> _Draft — please review and edit to reflect your own account before submitting._

This project was built with **Claude Code** (Anthropic) as a pair-programming
assistant, driven by me. AI assistance was used to scaffold the NestJS/Prisma and
Vite projects, draft the deep modules and validation rules, write the test
suites, and produce this documentation. Work followed a disciplined loop:

- The design was settled up front (PRD + ADRs) and sliced into vertical issues
  (walking skeleton → ingestion → validation engine → full rule set → approval →
  frontend → docs).
- Every deep module and rule was written **test-first** (red → green), and each
  slice was gated on the full suite passing and a clean lint/build before moving
  on. I reviewed the generated code, ran the tests and the app myself, and drove
  corrections (e.g. adjusting the e2e harness for the ESM-only embedded Postgres,
  and isolating tracer-bullet tests once the full rule set changed behavior).
- Where the assistant's first approach was wrong or over-built, I redirected it;
  the committed history reflects those iterations.

All architectural decisions were reviewed and accepted by me; the AI did not make
irreversible or unreviewed changes.
